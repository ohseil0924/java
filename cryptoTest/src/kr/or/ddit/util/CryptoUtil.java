...
public class CryptoUtil {
	...
	public static String encryptAeS256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		...
		//		동일한 평문 블로과 암호문의 블로그이 쌍이 발생하지 않도록
		// 		이전 단계 암복호화한 결과가 현 단계에 영향을 주는 운영모드 
		
		// Padding ==> 마지막 블록이 블록의 길이와 항상 딱 맞아 떨어지지 않기 때문에 
		//				부족한 만큼을 '0'으로 채우거나 임의의 비트로 채워 넣는것을 말한다.
		...
		// 옵션 종류에 맞게 초기화 한다.
		// 옵션 종류: ENCRYPT_MODE(암호화모드), DECRYPT_(복호화모드)
		// 
		
		//초기화 백터(Initial Vector,IV)
	...
	/**
	 * 
	 * encryptAES256()메서드로 암호화 한 내용을 복호화 하는 메서드
	 * @param str 복호화 할 문자열(암호화 된 문자열)
	 * @param key 암호키 문자열
	 * @return 복호화 된 문자열(원본의 문자열)
	 * @throws UnsupportedEncodingException 
	 * @throws InvalidAlgorithmParameterException 
	 * @throws InvalidKeyException 
	 * @throws NoSuchPaddingException 
	...
	public static String decryptAES256(String str, String key) throws UnsupportedEncodingException, InvalidKeyException, InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException, BadPaddingException {
		String iv = key.substring(0, 16);
		byte[] keyBytes = new byte[16];
		byte[] b = key.getBytes("UTF-8");
		int len = keyBytes.length;
