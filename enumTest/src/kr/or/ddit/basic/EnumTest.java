...
/*
	- enum(열거형) ==> 서로 관련있는 상수들의 집합을 나타낸다.
		...
		==> 열거형은 클래스처럼 독립된 java파일에 만들수 있고, 
			...
			클래스 내부에 내부 클래스 처럼 만들수 있다.
	
	- 열거형의 속성 및 메서드 
		1) name() ==> 열거형 상수의 이름을 문자열로 반환한다.
		2) ordinal() ==> 열거형 상수가 정의된 순서값(index값)을 반환한다.
						(0부터 시작)
		3) valueOf("열거형 상수명") ==> 지정된 열거형에서 
	...
	방법2) ==> 각각의 상수에 임의의 값을 설정할 수 있는 열거형 만들기 
		...
		private 열거형이름(자료형명 변수명, ...){///////////////////////////////////이거둘이 맞아야함
...
public class EnumTest {
	// 방법1로 열거형 선언하기
	public enum Color { RED, GREEN, BLUE } //끝에붙일때는 세미콜론안붙임	//이거 많이씀 
	...
	public enum Season{
		...
		//생성자
		Season(String span, int startMonth){ //위에줄과는 이름이 다른거임 
	...
	public static void main(String[] args) { //클래스가져오기
		//클래스 가져오기임 
		//다른클래스에서 가져온것과 비교하기 
...
//		System.out.println("RED => " + ConstTest.RED);
//		
//		if(ConstTest.GREEN == ConstTest.TWO){
//			System.out.println("...");
//		}else{
//			System.out.println("???");
//		}
...
//		if(myCol ==myCnt){
//			System.out.println("***"); //열거형이다르면 문법적으로 오류가 난다 .
//			
//		}
		...
		// switch문에서의 비교
		// switch문에서의 case에 상수명을 지정할 때는 '열거형이름' 을 생략하고 
		// 		'상수명'만 지정한다
		switch(myCnt){ //스위치문에서 사용할때는 열거형 앞에 count식의 단어를 생략함
			case ONE :
